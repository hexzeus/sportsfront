...
export default function BetsPage() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-black via-dark-blue to-slate-900 text-gray-200 p-4 sm:p-6 md:p-8 relative overflow-hidden">
            ...
            >
                ...
                {!loading && filteredBets.length === 0 && !error && (
                    <motion.p
                        className="text-center text-gray-400 text-xl sm:text-2xl"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.5 }}
                    >
                    ...
                    >
                        {filteredBets.map((bet) => (
                            <motion.div
                                key={bet.id}
                                id={`bet-${bet.id}`}
                                className="bg-gradient-to-br from-black to-slate-900 rounded-xl shadow-xl overflow-hidden transform transition-all duration-300 border border-blue-700 hover:border-gray-500"
                                initial={{ opacity: 0, y: 20 }}
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0, y: -20 }}
                                whileHover={{ scale: 1.03, boxShadow: "0 0 15px rgba(70, 130, 180, 0.3)" }}
                                whileTap={{ scale: 0.97 }}
                                layout
                            >
                                <div className="p-4 relative">
                                    <div className="absolute inset-0 bg-gradient-to-br from-blue-900/10 via-slate-900/10 to-gray-900/10 pointer-events-none"></div>
                                    {isValidUrl(bet.description) ? (
                                        <div className="flex flex-col items-center relative z-10">
                                            ...
                                            />
                                            <p className="text-gray-100 mt-4 text-lg sm:text-xl font-bold">
                                                Result:
                                ...
                                <div className="bg-black bg-opacity-50 p-4 flex justify-center space-x-4 items-center border-t border-blue-700">
                ...
                <div className="mt-10 sm:mt-14 md:mt-16 text-center">
                    <Link href="/">
                        <motion.span
                            className="inline-block bg-gradient-to-r from-blue-600 to-slate-600 text-gray-100 text-md sm:text-lg font-extrabold uppercase py-3 px-8 rounded-full shadow-2xl hover:from-blue-700 hover:to-slate-700 transition-all duration-300 ease-out transform-gpu"
